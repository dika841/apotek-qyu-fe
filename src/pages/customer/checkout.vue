...
const getBucket = async () => {
  ...
  const sum = data.map((x) => x.harga).reduce(function(a, b) { return a + b;}, 0)
  sumed.value = sum
};
...
const danaPay = () => {
  Swal.fire({
  text: 'Silahkan Scan',
  imageUrl: 'https://upload.wikimedia.org/wikipedia/commons/4/43/WhatsApp_click-to-chat_QR_code.png',
  imageWidth: 300,
  imageHeight: 300,
  imageAlt: 'Custom image',
})
          ...
          <div class="mt-8">
            <div class="flex flex-col space-y-4">
              <div class="flex flex-col space-x-4">
               <div v-for="(item, index) in listBucket" :key="index" class="flex flex-col mt-2 justify-between w-full border-b-2 p-2 border-black-300">
